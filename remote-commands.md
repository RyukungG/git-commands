...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > you did not create new branches on the remote from local.
...
6. Consider this situation:
   ...
   >It will raise an error because the remote branch contains code that you do not have locally. The solution is to git fetch first and then use git merge to merge you local code with the that you fetch from github.
...
8. To create a *second* remote repository for your local repo, the command to add a remote named "bitbucket" with the URL "https://bitbucket.org/your-username/git-commands" is:
   ```
   git remote add bitbucket https://bitbucket.org/pakornryu/git-commands
   ```
